...
label, keywords = 'milestone-experiments2', 'benchmark_testing-<del>_test1'
...
def diff_optimizer(curr_optimizer, x_train, y_test, activation_func, label=None, curr_epochs=MAX_EPOCHS):
    img_rows, img_cols, img_channels = x_train[0].shape
    nb_classes = y_test[0].shape[0]
    model = ResNet34((img_rows, img_cols, img_channels), nb_classes, activation_func)
...
if __name__ == '__main__':
    ...
    for optimizer in testing_optimizers:
        for activation in testing_activations:
            ...
            if optimizer == 'RAdam':
                score_returned, history = diff_optimizer(RAdam(lr=0.005), x_train, y_test, activation, label='RAdam')
            else:
                score_returned, history = diff_optimizer(optimizer, x_train, y_test, activation)
